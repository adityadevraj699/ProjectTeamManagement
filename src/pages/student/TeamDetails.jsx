...
import { motion, AnimatePresence } from "framer-motion"; // 👈 Ye add karo
...
export default function TeamDetails() {
  ...
  // Student side par data 'team' object mein hai
  navigate(`/projects/${team.projectId}/artifacts`, {
    state: {
      projectId: team.projectId, // Ensure your backend provides this
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-200 p-6 md:p-10 font-sans selection:bg-sky-500/30">
      <div className="max-w-7xl mx-auto">
        ...
        {/* --- NEW HEADER SECTION WITH BUTTON --- */}
      <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
        <h1 className="text-sm font-mono text-slate-500 uppercase tracking-[0.3em]">Project_Console</h1>
        
        <motion.button
          whileHover={{ scale: 1.05, boxShadow: "0 0 20px rgba(14, 165, 233, 0.3)" }}
          whileTap={{ scale: 0.95 }}
          onClick={handleGoToArtifacts}
          className="flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-sky-600 to-indigo-600 text-white rounded-xl font-bold shadow-lg text-xs uppercase tracking-widest transition-all border border-sky-400/20"
        >
